...
public class ClientCongresso {
	...
	public static void main(String[] args) {
		...
		String registryRemoteHost = null;
		...
		if (args.length != 1 && args.length != 2) {
			System.out.println("Sintassi: ClientCongresso NomeHostRegistryRemote [registryPort], registryPort intero");
			System.exit(1);
		}
		registryRemoteHost = args[0];
		if (args.length == 2) {
			try {
				registryRemotePort = Integer.parseInt(args[1]);
			} catch (Exception e) {
				System.out.println(
						"Sintassi: ClientCongresso NomeHostRegistryRemote [registryPort], registryPort intero");
				System.exit(1);
			}
		}
		...
		try {
			String completeRemoteRegistryName = "//" + registryRemoteHost + ":" + registryRemotePort + "/"
					+ registryRemoteName;
			RegistryRemoteTagClient registryRemote = (RegistryRemoteTagClient) Naming.lookup(completeRemoteRegistryName);

			System.out.println("Inserire TAG: ");
			tag = stdIn.readLine();
			String[] nomiServizi = registryRemote.cercaTag(tag);
			for (int i = 0; i < nomiServizi.length; i++) {
				System.out.println(i + ") " + nomiServizi[i]);
			}
			
			System.out.println("Inserire numero servizio richiesto: ");
			int servizioScelto = Integer.parseInt(stdIn.readLine());
			
			if(servizioScelto < 0 || servizioScelto >= nomiServizi.length) {
				System.out.println("Errore: numero non corretto.");
				System.exit(1);
			}
			
			ServerCongresso serverRMI = (ServerCongresso) registryRemote.cerca(nomiServizi[servizioScelto]);
			System.out.println("ClientRMI: Servizio \"" + nomiServizi[servizioScelto] + "\" connesso");
